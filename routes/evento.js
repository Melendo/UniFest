...
router.post('/inscribirse/:id', async (req, res) => {
    ...
    try {
        ...
        const queryEvento = 'SELECT activo, capacidad_máxima FROM eventos WHERE ID = ?';
        ...
        let mensaje = 'El evento está lleno, pero te hemos puesto en la lista de espera.';
...
router.post('/cancelar/:id', async (req, res) => {
    ...
    try {
        ...
        return res.status(200).json({ message: 'Tu inscripción ha sido cancelada.', titulo: 'Adiós' });
    } catch (error) {
...
router.get('/listadoAsistentes/:id', async (req, res) => {
    ...
    try {
        ...
        res.status(500).send('Hubo un error al obtener los datos de los asistentes');
    }
});
