  ...
  db.query(queryUsuario, [req.session.userId], (err, user) => {
    ...
    db.query(queryFacultad, [user[0].ID_facultad], (err, resfacultad) => {
      ...
      if (req.session.rol === 0) {
        queryEventosPasados =
          "SELECT eventos.título, eventos.fecha, eventos.ID FROM eventos JOIN inscripciones ON eventos.ID = inscripciones.ID_evento WHERE inscripciones.ID_usuario = ? AND inscripciones.estado = 'inscrito' AND inscripciones.activo = 1 AND eventos.fecha < NOW() AND eventos.activo = 1 ORDER BY eventos.fecha DESC";
      } else {
        queryEventosPasados =
          "SELECT eventos.título, eventos.fecha, eventos.ID FROM eventos WHERE ID_org = ? AND eventos.fecha < NOW() AND eventos.activo = 1 ORDER BY eventos.fecha DESC";
      }
